...
export function Brand() {
  return (
    <div className="flex items-center gap-2.5 select-none cursor-default">
      <span className="relative w-[22px] h-[22px] rounded-md overflow-hidden" style={{ background: "conic-gradient(from 200deg, var(--color-accent), var(--color-background), var(--color-accent))" }}>
    ...
    >
      <div className="container-x flex items-center justify-between gap-8 py-3.5">
        ...
        <div className="flex items-center gap-2.5">
          <ThemeToggle />
          <Link to="/contact" className="hidden sm:inline-flex items-center px-4 py-2.5 rounded-[10px] text-[13px] border border-border-strong hover:bg-surface-2 transition-all">
            Book a Call
          </Link>
          <Link to="/contact" className="inline-flex items-center px-4 py-2.5 rounded-[10px] text-[13px] font-medium bg-foreground text-background hover:shadow-[0_0_40px_var(--color-accent)] transition-all">
            Start a Project
          </Link>
        </div>
      </div>
